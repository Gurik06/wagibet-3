...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-65d63d8e";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-65d63d8e">'+'<span class="tile-face-65d63d8e tile-front-65d63d8e"></span>'+'<span class="tile-face-65d63d8e tile-back-65d63d8e"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-65d63d8e")){return;}
var back=tile.querySelector(".tile-back-65d63d8e");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-65d63d8e");if(isMine){tile.classList.add("boom-65d63d8e");}
if(dim){tile.classList.add("dimmed-65d63d8e");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-65d63d8e")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-65d63d8e");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-65d63d8e")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-65d63d8e");void panelEl.offsetWidth;panelEl.classList.add("jolt-65d63d8e");}
endRound();}
